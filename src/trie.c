...
int trie_replace(struct trie *self, const char *key, trie_replacer f, void *arg)
{
    struct trie *last;
    struct trieptr *parent;
    unsigned char *ukey = (unsigned char *)key;
...
int trie_visit(struct trie *self, const char *prefix, trie_visitor v, void *arg)
{
...
int trie_it_next(struct trie_it *it)
{
...
int trie_it_done(struct trie_it *it)
{
    return it->error || !it->stack.fill;
}

int trie_it_error(struct trie_it *it)
{
    return it->error;
}

void trie_it_free(struct trie_it *it)
{
